...
function makePhalanxPart(length, width, thick, hasFrontClevis, frontScale = 1.0) {
  const body = roundedRect(width * 0.72, length, Math.max(2.0 * scale, width * 0.12))
...
wristRotated.push({ name: "Palm", shape: palm.rotateAroundAxis([1, 0, 0], wristPitch, [0, 0, 0]) });

for (const p of handParts) {
  wristRotated.push({
    name: p.name,
    shape: p.shape.rotateAroundAxis([1, 0, 0], wristPitch, [0, 0, 0]),
  });
}
