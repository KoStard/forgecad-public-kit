...
| `filletEdge()` / `chamferEdge()` | Partial | Yes | Compiler-owned tracked-edge subset: vertical edges from compile-covered `box()` bodies and `rectangle(...).extrude(...)` flows, plus untouched sibling vertical edges after earlier supported edge-finish rewrites on the same body; shell/boolean/hole/cut descendants and the already-finished merged edge itself are still outside the defended exact subset |
