...
function filledPolygon(pts: [number, number][], r: number, g: number, b: number, opacity: number): string {
  ...
  const lines: string[] = [];
  lines.push('q');
  const bg: [number, number, number] = [0.102, 0.102, 0.180];
...
function computeBounds(meta: SketchConstraintMeta): { min: [number, number]; max: [number, number] } {
  ...
  for (const line of meta.edges.lines) { expand(line.a[0], line.a[1]); expand(line.b[0], line.b[1]); }
  ...
  for (const arc of meta.edges.arcs) {
    expand(arc.start[0], arc.start[1]); expand(arc.end[0], arc.end[1]);
    expand(arc.center[0] - arc.radius, arc.center[1] - arc.radius);
    expand(arc.center[0] + arc.radius, arc.center[1] + arc.radius);
  }
  for (const pt of meta.edges.points) { expand(pt.pos[0], pt.pos[1]); }
  for (const line of meta.construction.lines) { expand(line.a[0], line.a[1]); expand(line.b[0], line.b[1]); }
  for (const circle of meta.construction.circles) {
    expand(circle.center[0] - circle.radius, circle.center[1] - circle.radius);
    expand(circle.center[0] + circle.radius, circle.center[1] + circle.radius);
  }
  for (const arc of meta.construction.arcs) { expand(arc.start[0], arc.start[1]); expand(arc.end[0], arc.end[1]); }

  for (const c of meta.constraints) {
    for (const ann of c.annotations) {
      if (ann.kind === 'symbol') { expand(ann.position[0], ann.position[1]); }
        ...
        expand(ann.from[0], ann.from[1]); expand(ann.to[0], ann.to[1]);
      } else if (ann.kind === 'angle-arc') {
        expand(ann.center[0] - ann.radius - 2, ann.center[1] - ann.radius - 2);
        expand(ann.center[0] + ann.radius + 2, ann.center[1] + ann.radius + 2);
      } else if (ann.kind === 'text') { expand(ann.position[0], ann.position[1]); }
    }
  }

  if (!isFinite(bounds.min[0])) { bounds.min = [-10, -10]; bounds.max = [10, 10]; }
  return bounds;
}
  ...
  switch (symbol) {
    ...
    case 'fixed':
      ...
      for (let i = -0.7; i <= 0.8; i += 0.5)
        cmds.push(lineCmd(px + S * i, py + S * 0.3, px + S * (i - 0.3), py + S * 0.8));
      break;
    case 'midpoint': {
      const pts: [number, number][] = [[px, py - S * 0.5], [px + S * 0.4, py], [px, py + S * 0.5], [px - S * 0.4, py]];
    ...
    case 'ccw': {
      ...
      for (let i = 0; i <= 12; i++) { const a = (i / 12) * Math.PI * 1.5; arcPts.push([px + r * Math.cos(a), py + r * Math.sin(a)]); }
...
function renderDimensionPdf(tx: SketchToPdfTransform, ann: Extract<AnnotationElement, { kind: 'dimension' }>, color: [number, number, number]): string {
  ...
  const arrowLen = Math.min(1.0, len * 0.15), arrowW = arrowLen * 0.35;
  ...
  if (Math.abs(o) > 0.5) {
    cmds.push(`${f(0.3)} w`);
    const [e1ax, e1ay] = toPage(tx, e1a[0], e1a[1]); const [e1bx, e1by] = toPage(tx, e1b[0], e1b[1]);
    const [e2ax, e2ay] = toPage(tx, e2a[0], e2a[1]); const [e2bx, e2by] = toPage(tx, e2b[0], e2b[1]);
    cmds.push(lineCmd(e1ax, e1ay, e1bx, e1by), lineCmd(e2ax, e2ay, e2bx, e2by));
  }

  cmds.push(`${f(0.5)} w`);
  const [p1x, p1y] = toPage(tx, p1[0], p1[1]); const [p2x, p2y] = toPage(tx, p2[0], p2[1]);
...
function renderAngleArcPdf(tx: SketchToPdfTransform, ann: Extract<AnnotationElement, { kind: 'angle-arc' }>, color: [number, number, number]): string {
  ...
  const cmds: string[] = ['q', strokeColor(color[0], color[1], color[2]), fillColor(color[0], color[1], color[2]), `${f(0.5)} w`, polylineStroke(pts)];
...
function buildSketchPdfContent(meta: SketchConstraintMeta, tx: SketchToPdfTransform): string {
  ...
  // Background
  cmds.push('q', fillColor(0.102, 0.102, 0.180), `${f(0)} ${f(0)} ${f(tx.pageWidth)} ${f(tx.pageHeight)} re f`, 'Q');
  ...
  for (const line of meta.edges.lines) {
    const [ax, ay] = toPage(tx, line.a[0], line.a[1]); const [bx, by] = toPage(tx, line.b[0], line.b[1]);
