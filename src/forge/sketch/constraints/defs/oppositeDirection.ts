...
declare module '../types' {
  interface ConstraintTypeMap {
     ...
     * The anti-parallel requirement is enforced via presolve/solve and a
     * one-sided residual penalty when co-directional.
     */
...
registerConstraint<'oppositeDirection', ConstraintTypeMap['oppositeDirection']>({
  ...
  presolve(c, { lines, points }) {
    flipIfCoDirectional(c, lines, points);
  },
  ...
  solve(c, { lines, points, tolerance }) {
    ...
    // Target: a's angle + π (opposite direction)
    const target = angleOfLine(a1, a2) + Math.PI;
    ...
    const cos = Math.cos(target), sin = Math.sin(target);
    if (b1.fixed) {
      b2.x = b1.x + cos * len; b2.y = b1.y + sin * len;
    } else if (b2.fixed) {
      b1.x = b2.x - cos * len; b1.y = b2.y - sin * len;
    } else {
      const mid = midpoint(b1, b2);
      b1.x = mid[0] - cos * len / 2; b1.y = mid[1] - sin * len / 2;
      b2.x = mid[0] + cos * len / 2; b2.y = mid[1] + sin * len / 2;
    }
    return err;
  },
  ...
  residual(c, { lines, points }) {
    ...
    // When co-directional (dot > 0), add a penalty to drive LM toward anti-parallel.
    if (dot > 0) {
      return [cross + 2 * Math.sign(cross || 1)];
    }
    return [cross];
  },
  ...
  jacobian(c, { lines, points }) {
    ...
    const residual = dot > 0
      ? cross + 2 * Math.sign(cross || 1)
      : cross;
...
/**
 * If `b` points the same way as `a`, flip `b`'s endpoints so it points opposite.
 */
function flipIfCoDirectional(
  c: ConstraintTypeMap['oppositeDirection'],
