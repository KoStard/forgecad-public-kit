...
interface ConstraintBuilderMethods {
  ...
  importRectangle(
    ...
    top: LineId;
    left: LineId;
    points: [PointId, PointId, PointId, PointId];
  };
...
type SdfNode =
  ...
  | SdfVoronoiNode
  | SdfCustomNode;
declare const SHEET_METAL_EDGES: readonly ['top', 'right', 'bottom', 'left'];
...
declare class Shape {
  ...
  attachTo(
    ...
    selfAnchor?: PlacementAnchorLike,
    offset?: [number, number, number],
  ): Shape;
  /**
   * Place this shape on a face of a parent shape.
...
type JointViewAxis = [number, number, number];
...
declare class ImportedAssembly {
  ...
  constructor(_assembly: Assembly, _refs?: PlacementReferences, _offset?: readonly [number, number, number]);
 ...
 */
declare function joint(name: string, shape: Shape, pivot: [number, number, number], opts?: RevoluteJointOpts): Shape;
interface BomOpts {
  /**
 ...
 */
declare function draft(shape: ShapeArg$1, angleDeg: number, pullDirection?: [number, number, number], neutralPlaneOffset?: number): Shape;
/**
...
type Vec3$3 = [number, number, number];
...
type PointArg$1 = [number, number] | [number, number, number] | Point2D;
interface DimOpts {
  offset?: number;
  label?: string;
  color?: string;
...
declare function filletEdge(shape: ShapeArg$2, edge: EdgeRef, radius: number, quadrant?: [number, number], segments?: number): Shape;
/** Bevel a named edge of a shape with a 45-degree chamfer of the given size. Requires a compile-covered target. */
declare function chamferEdge(shape: ShapeArg$2, edge: EdgeRef, size: number, quadrant?: [number, number]): Shape;
...
type Vec3$4 = [number, number, number];
...
declare function stroke(points: [number, number][], width: number, join?: 'Round' | 'Square'): Sketch;
...
declare function mirrorCopy(shape: ShapeArg$3, normal: [number, number, number]): Shape;
...
type Vec3$5 = [number, number, number];
...
type Vec3$6 = [number, number, number];
 ...
 */
declare function transitionCurveFromPoints(
  startPoint: Vec3$6,
  startTangent: Vec3$6,
  endPoint: Vec3$6,
  endTangent: Vec3$6,
...
declare const verify: {
   ...
   */
  faceAt(label: string, face: FaceRefLike, expectedPos: [number, number, number], toleranceMm?: number): void;
  /**
   ...
   */
  boundingBoxSize(label: string, shape: ShapeLike$1, expectedSize: [number, number, number], tolerance?: number): void;
};
...
declare function highlight(point: [number, number, number], opts?: HighlightOptions): void;
declare function highlight(edge: [[number, number, number], [number, number, number]], opts?: HighlightOptions): void;
...
declare namespace sdf {
  ...
  export function smoothDifference(
    a: SdfShape,
    b: SdfShape,
    options: {
      radius: number;
  ...
  export function smoothIntersection(
    a: SdfShape,
    b: SdfShape,
    options: {
      radius: number;
