...
async function main() {
  ...
  for (let run = 0; run < RUNS; run++) {
    ...
    const _result = sk.solve({ iterations: 200, restarts: 12 });
  ...
  for (const c of constrainTimes) {
    ...
    console.log(
      `  #${String(c.idx).padStart(3)} ${c.type.padEnd(22)} ${c.ms.toFixed(0).padStart(5)}ms  cum=${cumulative.toFixed(0).padStart(6)}ms  ${bar}`,
    );
  }
}
