  ...
  ConstrainedSketchBuilder.prototype.constrain = function (constraint: any) {
    ...
    if (ms > 2) {
      ...
      const { maxError } = solveConstraints(
        {
          ...def,
          points: def.points.map((p: any) => ({ ...p })),
          lines: def.lines.map((l: any) => ({ ...l })),
          circles: def.circles.map((c: any) => ({ ...c })),
          arcs: (def.arcs ?? []).map((a: any) => ({ ...a })),
        },
        { iterations: 1, tolerance: DEFAULT_TOLERANCE, restarts: 1, warmStartIterations: 0 },
      );
      console.log(
        `  #${String(cIdx).padStart(3)} ${constraint.type.padEnd(22)} ${ms.toFixed(0).padStart(5)}ms  postSolveErr=${maxError.toFixed(4)}  ${maxError <= DEFAULT_TOLERANCE ? '✓ converged' : '✗ NOT converged'}`,
      );
    }
    return result;
  };
