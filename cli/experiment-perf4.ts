#!/usr/bin/env node
/**
 * Profile WHERE time is spent inside a single constrain() call.
 * Instruments linearizeSystem, solveLM, etc.
 */
...
async function main() {
  ...
  // Build the spectrogram, measure constraint #48 in detail
  ...
  ConstrainedSketchBuilder.prototype.constrain = function (constraint: any) {
    ...
    if (cIdx === 48) {
      console.log(`Constraint #48 (${constraint.type}): ${(t1-t0).toFixed(1)}ms`);
    }
    return result;
  };
  ...
  const t1 = performance.now();
  console.log(`Final solve: ${(t1-t0).toFixed(0)}ms, maxErr=${result.constraintMeta.maxError.toFixed(4)}`);
