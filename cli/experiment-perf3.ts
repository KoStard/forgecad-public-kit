#!/usr/bin/env node
/**
 * Profile what happens inside decomposeAndSolve for the slow constraints.
 * Checks: how many components, how many variables per component, Jacobian size.
 */
...
async function main() {
  ...
  ConstrainedSketchBuilder.prototype.constrain = function (constraint: any) {
    ...
    const nPts = self.points.length;
    const nFixed = self.points.filter((p: any) => p.fixed).length;
    const nConstraints = self.constraints.length; // before adding
    const t0 = performance.now();
    const result = origConstrain.call(this, constraint);
    const t1 = performance.now();
    const ms = t1 - t0;
    if (ms > 2) {
      console.log(`  #${String(cIdx).padStart(3)} ${constraint.type.padEnd(22)} ${ms.toFixed(0).padStart(5)}ms  pts=${nPts} free=${nPts - nFixed} constraints=${nConstraints + 1}`);
    }
    return result;
  };

  console.log('Slow constraints with system size info:\n');
